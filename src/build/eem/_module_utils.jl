# !!!! Incorporate into SLiDE/utils... location tbd.
...
"""
"""
function operate_with_2(df::DataFrame, conversion::DataFrame; id=[], keepinput::Bool=false)
    ...
    col = [setdiff(findindex(df), utx); [:units,:value]; sort([utx; val])]

    # !!!! Function that isolates key vs units/value.
    # BUT THIS WON'T WORK FOR HOW WE DEFINE THINGS FOR INDEXJOIN AND THEREFORE FOR ref/ind.
    if !isempty(id)
        val = vcat([val[occursin.(k, val)] for k in id]...,)
        utx = vcat([utx[occursin.(k, utx)] for k in id]...,)
    end
