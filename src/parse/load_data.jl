...
# Returns
- `filepropertynames::Array{String,1}`: List of yaml files
...
function write_yaml(path, file::XLSXInput)
    ...
    filepropertynames = joinpath.(path, string.(propertynames(df_all), ".yml"))
...
"""
    ...
    run_yaml(filepropertynames::Array{String,1})
...
# Arguments
- `filename::String` or `filepropertynames::Array{String,1}`: yaml file name (or list of propertynames) to run

# Return
- `filename::String` or `filepropertynames::Array{String,1}`: yaml file name (or list of propertynames) that
...
function run_yaml(filepropertynames::Array{String,1})
    println("\nSTANDARDIZE DATA FILES:")
    filepropertynames = [run_yaml(f) for f in filepropertynames]
    filepropertynames = filepropertynames[filepropertynames .!== nothing]
    if length(filepropertynames) > 0
