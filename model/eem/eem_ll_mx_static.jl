##################################################
#
# Replication of windc-3.0 labor leisure (ll) in julia with counterfactual testing
#
##################################################
...
@NLparameter(cge, es_inv == 5); # Final consumption 
...
@mapping(cge,income_ra[r in set[:r]],
         ...
         - (
             ...
             + PLS[r]*lbr0[r]
             + PLS[r]*lsr0[r]
             + sum(RK[(r,s)]*ksrs_m[r,s] + RKX[(r,s)]*ks_x[r,s] for s in set[:s] if ((r,s) in sset[:PK]))
