...
#sectors
@variable(cge, Y[r in regions, s in sectors; (r, s) in set[:Y]] >= sv, start = 1);
@variable(cge, X[r in regions, g in goods; (r, g) in set[:X]] >= sv, start = 1);
@variable(cge, A[r in regions, g in goods; (r, g) in set[:A]] >= sv, start = 1);
@variable(cge, C[r in regions] >= sv, start = 1);
@variable(cge, MS[r in regions, m in margins] >= sv, start = 1);

#commodities:
@variable(cge, PA[r in regions, g in goods; (r, g) in set[:PA]] >= sv, start = 1); # Regional market (input)
@variable(cge, PY[r in regions, g in goods; (r, g) in set[:PY]] >= sv, start = 1); # Regional market (output)
@variable(cge, PD[r in regions, g in goods; (r, g) in set[:PD]] >= sv, start = 1); # Local market price
@variable(cge, PN[g in goods] >= sv, start =1); # National market
@variable(cge, PL[r in regions] >= sv, start = 1); # Wage rate
@variable(cge, PK[r in regions, s in sectors; (r, s) in set[:PK]] >= sv, start =1); # Rental rate of capital ###
...
@NLexpression(cge,CVA[r in regions,s in sectors],
  PL[r]^alpha_kl[r,s] * (haskey(PK, (r, s)) ? PK[r, s] : 1.0) ^ (1-alpha_kl[r,s]) );
...
@NLexpression(cge,AK[r in regions,s in sectors],
  kd0[r,s] * CVA[r,s] / (haskey(PK, (r, s)) ? PK[r, s] : 1.0) );
...
@NLexpression(cge,RX[r in regions,g in goods],
  (alpha_x[r,g]*PFX^(1 + et_x[r,g])+alpha_n[r,g]*PN[g]^(1 + et_x[r,g])+alpha_d[r,g]*(haskey(PD, (r, g)) ? PD[r,g] : 1.0)^(1 + et_x[r,g]))^(1/(1 + et_x[r,g])) );
...
@NLexpression(cge,AD[r in regions,g in goods],
  xd0[r,g] * ((haskey(PD, (r, g)) ? PD[r,g] : 1.0) / (RX[r,g]))^et_x[r,g] );

# CES function for tradeoff between national and domestic market
@NLexpression(cge,CDN[r in regions,g in goods],
  (theta_n[r,g]*PN[g]^(1-es_d[r,g])+(1-theta_n[r,g])*(haskey(PD, (r, g)) ? PD[r,g] : 1.0)^(1-es_d[r,g]))^(1/(1-es_d[r,g])) );
...
@NLexpression(cge,DD[r in regions,g in goods],
  dd0[r,g]*(CDN[r,g]/(haskey(PD, (r, g)) ? PD[r,g] : 1.0))^es_d[r,g]*(CDM[r,g]/CDN[r,g])^es_f[r,g] );
...
@NLexpression(cge,CD[r in regions,g in goods],
  cd0[r,g]*PC[r] / (haskey(PA, (r, g)) ? PA[r,g] : 1.0) );
...
@mapping(cge,profit_y[r in regions, s in sectors; (r, s) in set[:Y]],
# cost of intermediate demand
        sum((haskey(PA, (r, g)) ? PA[r,g] : 1.0) * id0[r,g,s] for g in goods) 
# cost of labor inputs
        + PL[r] * AL[r,s]
# cost of capital inputs 
        + (haskey(PK, (r, s)) ? PK[r,s] : 1.0)* AK[r,s]
        - 
# revenue from sectoral supply (take note of r/s/g indices on ys0)                
        sum((haskey(PY, (r, g)) ? PY[r,g] : 1.0)  * ys0[r,s,g] for g in goods) * (1-ty[r,s])
);

@mapping(cge,profit_x[(r, g) in set[:X]],
# output 'cost' from aggregate supply
         (haskey(PY, (r, g)) ? PY[r,g] : 1.0) * s0[r,g] 
        - (
...
# revenues from domestic market
        + (haskey(PD, (r, g)) ? PD[r,g] : 1.0) * AD[r,g]
        )
);
...
# costs from domestic market                  
        + (haskey(PD, (r, g)) ? PD[r,g] : 1.0) * DD[r,g] 
...
# revenues from regional market based on armington supply               
        (haskey(PA, (r, g)) ? PA[r,g] : 1.0) * (1-ta[r,g]) * a0[r,g] 
...
# costs of inputs - computed as final demand times regional market prices
        sum((haskey(PA, (r, g)) ? PA[r,g] : 1.0) * CD[r,g] for g in goods)
        - 
...
# provision of margins to domestic market    
        + sum((haskey(PD, (r, gm)) ? PD[r, gm] : 1.0) * dm0[r,gm,m] for gm in goods_margins)
        - 
# total margin demand    
...
# final demand        
        + C[r] * CD[r,g]
# intermediate demand        
        + sum((haskey(Y, (r, s)) ? Y[r,s] : 1.0) * id0[r,g,s] for s in sectors if (r,s) in set[:Y])
        )
);

@mapping(cge,market_py[(r, g) in set[:PY]],
# sectoral supply
        sum((haskey(Y, (r, s)) ? Y[r,s] : 1.0) *ys0[r,s,g] for s in sectors)
# household production (exogenous)        
        + yh0[r,g]
        - 
# aggregate supply (akin to market demand)                
       (haskey(X, (r, g)) ? X[r,g] : 1.0) * s0[r,g]
);

@mapping(cge,market_pd[(r, g) in set[:PD]],
# aggregate supply
        (haskey(X, (r, g)) ? X[r,g] : 1.0)  * AD[r,g] 
        - ( 
# demand for local market          
        (haskey(A, (r, g)) ? A[r,g] : 1.0) * DD[r,g]
...
# supply to the national market
        sum((haskey(X, (r, g)) ? X[r,g] : 1.0)  * AN[r,g] for r in regions)
        - ( 
# demand from the national market 
        sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * DN[r,g] for r in regions)
...
# demand for labor in all sectors        
        sum((haskey(Y, (r, s)) ? Y[r,s] : 1.0) * AL[r,s] for s in sectors)
);
...
# margin supply 
        ...
        - 
# margin demand        
        sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * md0[r,m,g] for g in goods)
);
...
# supply of exports     
        + sum((haskey(X, (r, g)) ? X[r,g] : 1.0)  * AX[r,g] for r in regions for g in goods)
# supply of re-exports        
        + sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * rx0[r,g] for r in regions for g in goods if (r,g) in set[:A])
        - 
# import demand                
        sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * MD[r,g] for r in regions for g in goods if (r,g) in set[:A])
);
...
# capital income        
        + sum((haskey(PK, (r, s)) ? PK[(r,s)] : 1.0) * kd0[r,s] for s in sectors)
# provision of household supply          
        + sum( (haskey(PY, (r, g)) ? PY[r,g] : 1.0) * yh0[r,g] for g in goods)
...
# government and investment provision        
        - sum((haskey(PA, (r, g)) ? PA[r,g] : 1.0) * (g0[r,g] + i0[r,g]) for g in goods)
# import taxes - assumes lumpsum recycling
        + sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * MD[r,g] * PFX * tm[r,g] for g in goods if (r,g) in set[:A])
# taxes on intermediate demand - assumes lumpsum recycling
        + sum((haskey(A, (r, g)) ? A[r,g] : 1.0) * a0[r,g]*(haskey(PA, (r, g)) ? PA[r,g] : 1.0)*ta[r,g] for g in goods if (r,g) in set[:A])
# production taxes - assumes lumpsum recycling  
        + sum( (haskey(Y, (r, s)) ? Y[r,s] : 1.0) * ys0[r,s,g] * ty[r,s] for s in sectors, g in goods)
        )
);
