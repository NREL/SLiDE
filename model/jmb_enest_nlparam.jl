...
@NLparameter(cge, fe_bar[r in regions, s in sectors] == sum(value(id0_p[r, g, s]) for g in fe));
@NLparameter(cge, en_bar[r in regions, s in sectors] == sum(value(id0_p[r, g, s]) for g in en));
@NLparameter(cge, ne_bar[r in regions, s in sectors] == sum(value(id0_p[r, g, s]) for g in ne));
@NLparameter(cge, vaen_bar[r in regions, s in sectors] == value(en_bar[r, s]) + value(va_bar[r, s]));
@NLparameter(cge, klem_bar[r in regions, s in sectors] == value(vaen_bar[r, s]) + value(ne_bar[r, s]));

@NLparameter(cge, alpha_l[r in regions, s in sectors] == value(ld0_p[r, s]) / value(va_bar[r, s]));
@NLparameter(cge, alpha_k[r in regions, s in sectors] == value(kd0_p[r, s]) / value(va_bar[r, s]));
@NLparameter(cge, theta_fe[r in regions, g in fe, s in sectors] == value(id0_p[r, g, s]) / value(fe_bar[r, s]));
@NLparameter(cge, theta_en[r in regions, g in en, s in sectors] == value(id0_p[r, g, s]) / value(en_bar[r, s]));
@NLparameter(cge, theta_ele[r in regions, s in sectors] == sum(value(id0_p[r, g, s]) for g in ele) / value(en_bar[r, s]));
@NLparameter(cge, theta_ne[r in regions, g in ne, s in sectors] == value(id0_p[r, g, s]) / value(ne_bar[r, s]));

@NLparameter(cge, theta_va[r in regions, s in sectors] == value(va_bar[r, s]) / (value(va_bar[r, s]) + value(en_bar[r, s])));
@NLparameter(cge, theta_ene[r in regions, s in sectors] == value(en_bar[r, s]) / (value(va_bar[r, s]) + value(en_bar[r, s])));
@NLparameter(cge, theta_kle[r in regions, s in sectors] == value(en_bar[r, s]) / (value(vaen_bar[r, s]) + value(klem_bar[r, s])));
@NLparameter(cge, theta_mm[r in regions, s in sectors] == value(en_bar[r, s]) / (value(ne_bar[r, s]) + value(klem_bar[r, s])));

@NLparameter(cge, esub_fe == 0.5);
@NLparameter(cge, esub_ele == 0.1);
@NLparameter(cge, esub_ve == 0.5);
@NLparameter(cge, esub_ne == 0.2);
@NLparameter(cge, esub_klem == 0.3);
...
replace_nan_inf(alpha_l)
replace_nan_inf(alpha_k)
replace_nan_inf(theta_fe)
replace_nan_inf(theta_en)
replace_nan_inf(theta_ele)
replace_nan_inf(theta_ne)
replace_nan_inf(theta_va)
replace_nan_inf(theta_ene)
replace_nan_inf(theta_kle)
replace_nan_inf(theta_mm)
...
en_bar0 = Dict();
va_bar0 = Dict();
[en_bar0[r,s] = value(en_bar[r,s]) for r in regions for s in sectors];
[va_bar0[r,s] = value(va_bar[r,s]) for r in regions for s in sectors];
    
sub_set_pe = filter(x -> en_bar0[x[1], x[2]] != 0.0, combvec(regions, sectors));
sub_set_pva = filter(x -> va_bar0[x[1], x[2]] != 0.0, combvec(regions, sectors));
sub_set_va = sub_set_pva;
sub_set_e = sub_set_pe;
...
@variable(cge,E[(r, s) in sub_set_e] >= sv, start=1);
@variable(cge,VA[(r,s) in sub_set_va] >= sv, start=1);

##energy nesting commodities
@variable(cge,PE[(r, s) in sub_set_pe] >= sv,start=1);
@variable(cge,PVA[(r, s) in sub_set_pva] >= sv,start=1);
...
@NLexpression(cge,CFE[r in regions, s in sectors],
        ( sum( theta_fe[r,g,s] * (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) ^ (1-esub_fe) for g in fe ) ) ^ (1/(1-esub_fe))
);

@NLexpression(cge,CEN[r in regions, s in sectors],
        ( sum( theta_ele[r,s] * (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) ^ (1-esub_ele) for g in ele ) + ((1-theta_ele[r,s]) * CFE[r,s] ^ (1-esub_ele)) ) ^ (1/(1-esub_ele))
);
...
@NLexpression(cge,CNE[r in regions, s in sectors],
        ( sum( theta_ne[r,g,s] * (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) ^ (1-esub_ne) for g in ne ) ) ^ (1/(1-esub_ne))
);
...
@NLexpression(cge,IDA_ne[r in regions, g in ne, s in sectors],
        id0_p[r, g, s] * ( CNE[r,s] / (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) ) ^ esub_ne
);

@NLexpression(cge,IDA_ele[r in regions, g in ele, s in sectors],
        id0_p[r, g, s] * ( CEN[r,s] / (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0))  ^ esub_ele
);

@NLexpression(cge,IDA_fe[r in regions, g in fe, s in sectors],
        id0_p[r, g, s] * (( CEN[r,s] / CFE[r,s] ) ^ esub_ele) * (( CFE[r,s] / (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) ) ^ esub_fe)
);
...
@mapping(cge,profit_va[(r, s) in sub_set_va],
        PL[r] * AL[r,s] + (haskey(PK.lookup[1], (r, s)) ? PK[(r, s)] : 1.0) * AK[r,s]
        -
        (haskey(PVA.lookup[1], (r, s)) ? PVA[(r, s)] : 1.0) * va_bar[r,s]
);

@mapping(cge,profit_e[(r, s) in sub_set_e],
        sum( (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) * IDA_ele[r,g,s] for g in ele )
        + sum( (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) * IDA_fe[r,g,s] for g in fe )
        -
...
@mapping(cge,profit_y[(r, s) in sub_set_y],
        ...
        + sum( (haskey(PA.lookup[1], (r, g)) ? PA[(r, g)] : 1.0) * IDA_ne[r,g,s] for g in ne )
        -
        sum( (haskey(PY.lookup[1], (r, g)) ? PY[(r, g)] : 1.0) * blueNOTE[:ys0][r,s,g] for g in goods ) * (1-blueNOTE[:ty][r,s])
);
...
@mapping(cge,market_pe[(r, s) in sub_set_pe],
         (haskey(E.lookup[1], (r, s)) ? E[(r, s)] : 1.0) * en_bar[r,s]
         - (haskey(Y.lookup[1], (r, s)) ? Y[(r, s)] : 1.0) * IE[r,s]
);

@mapping(cge,market_pva[(r, s) in sub_set_pva],
         (haskey(VA.lookup[1], (r, s)) ? VA[(r, s)] : 1.0) * va_bar[r,s]
         - (haskey(Y.lookup[1], (r, s)) ? Y[(r, s)] : 1.0) * IVA[r,s]
);
...
@mapping(cge,market_pa[(r, g) in sub_set_pa],
        (haskey(A.lookup[1], (r, g)) ? A[(r, g)] : 1.0) * blueNOTE[:a0][r,g]
        - (
                blueNOTE[:g0][r,g] + blueNOTE[:i0][r,g]
                + C[r] * CD[r,g]
                + sum((haskey(Y.lookup[1], (r, s)) ? Y[(r, s)] : 1.0) * IDA_ne[r,g,s] for s in sectors if (y_check[r,s] > 0) && (g in ne) )
                + sum((haskey(Y.lookup[1], (r, s)) ? Y[(r, s)] : 1.0) * IDA_ele[r,g,s] for s in sectors if (y_check[r,s] > 0) && (g in ele))
                + sum((haskey(Y.lookup[1], (r, s)) ? Y[(r, s)] : 1.0) * IDA_fe[r,g,s] for s in sectors if (y_check[r,s] > 0) && (g in fe))
        )
);
