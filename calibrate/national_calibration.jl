...
# Set all negative pce values in :fd0 to zero, but others can remain.
io[:fd0][ .&(io[:fd0][:,:fd] .== "pce", io[:fd0][:,:value] .< 0), :value] .= 0

cal = Dict();

# !!!! should this be done in io.jl / to the io DataFrames?
skpneg = [:fd0, :ta0, :tm0]
...
Obj_Diagnostics[:ys0_withkey] = sum(abs(cal[:ys0][j,i]) * (JuMP.value(ys0_est[j,i]) / cal[:ys0][j,i] - 1)^2 for i in set[:i] for j in set[:j] if haskey(cal[:ys0], (j, i))) ;
Obj_Diagnostics[:id0_withkey] = sum(abs(cal[:id0][i,j]) * (JuMP.value(id0_est[i,j]) / cal[:id0][i,j] - 1)^2 for i in set[:i] for j in set[:j] if haskey(cal[:id0], (i, j))) ;
Obj_Diagnostics[:fs0_withkey] = sum(abs(cal[:fs0][i]) * (JuMP.value(fs0_est[i]) / cal[:fs0][i] - 1)^2 for i in set[:i] if haskey(cal[:fs0], i)) ;
Obj_Diagnostics[:ms0_withkey] = sum(abs(cal[:ms0][i,m]) * (JuMP.value(ms0_est[i,m]) / cal[:ms0][i,m] - 1)^2 for i in set[:i] for m in set[:m] if haskey(cal[:ms0], (i, m))) ;
Obj_Diagnostics[:y0_withkey] = sum(abs(cal[:y0][i]) * (JuMP.value(y0_est[i]) / cal[:y0][i] - 1)^2 for i in set[:i] if haskey(cal[:y0], i)) ;
Obj_Diagnostics[:fd0_withkey] = sum(abs(cal[:fd0][i,fd]) * (JuMP.value(fd0_est[i,fd]) / cal[:fd0][i,fd] - 1)^2 for i in set[:i] for fd in set[:fd] if haskey(cal[:fd0], (i, fd))) ;
Obj_Diagnostics[:va0_withkey] = sum(abs(cal[:va0][va,j]) * (JuMP.value(va0_est[va,j]) / cal[:va0][va,j] - 1)^2 for va in set[:va] for j in set[:j] if haskey(cal[:va0], (va, j))) ;
Obj_Diagnostics[:a0_withkey] = sum(abs(cal[:a0][i]) * (JuMP.value(a0_est[i]) / cal[:a0][i] - 1)^2  for i in set[:i] if haskey(cal[:a0], i)) ;
Obj_Diagnostics[:x0_withkey] = sum(abs(cal[:x0][i]) * (JuMP.value(x0_est[i]) / cal[:x0][i] - 1)^2  for i in set[:i] if haskey(cal[:x0], i)) ;
Obj_Diagnostics[:m0_withkey] = sum(abs(cal[:m0][i]) * (JuMP.value(m0_est[i]) / cal[:m0][i] - 1)^2  for i in set[:i] if haskey(cal[:m0], i)) ;
Obj_Diagnostics[:md0_withkey] = sum(abs(cal[:md0][m,i]) * (JuMP.value(md0_est[m,i]) / cal[:md0][m,i] - 1)^2 for m in set[:m] for i in set[:i] if haskey(cal[:md0], (m, i)))  ;

Obj_Diagnostics[:ys0_nokey] = penalty_nokey * sum(JuMP.value(ys0_est[j,i]) for i in set[:i] for j in set[:j] if !haskey(cal[:ys0], (j, i))) ;
Obj_Diagnostics[:id0_nokey] = penalty_nokey * sum(JuMP.value(id0_est[i,j])  for i in set[:i] for j in set[:j] if !haskey(cal[:id0], (i, j))) ;
Obj_Diagnostics[:fs0_nokey] = penalty_nokey * sum(JuMP.value(fs0_est[i])  for i in set[:i] if !haskey(cal[:fs0], i)) ;
Obj_Diagnostics[:ms0_nokey] = penalty_nokey * sum(JuMP.value(ms0_est[i,m])  for i in set[:i] for m in set[:m] if !haskey(cal[:ms0], (i, m))) ;
Obj_Diagnostics[:y0_nokey] = penalty_nokey * sum(JuMP.value(y0_est[i]) for i in set[:i] if !haskey(cal[:y0], i)) ;
Obj_Diagnostics[:fd0_nokey] = penalty_nokey * sum(JuMP.value(fd0_est[i,fd]) for i in set[:i] for fd in set[:fd] if !haskey(cal[:fd0], (i, fd))) ;
Obj_Diagnostics[:va0_nokey] = penalty_nokey * sum(JuMP.value(va0_est[va,j]) for va in set[:va] for j in set[:j] if !haskey(cal[:va0], (va, j))) ;
Obj_Diagnostics[:a0_nokey] = penalty_nokey * sum(JuMP.value(a0_est[i]) for i in set[:i] if !haskey(cal[:a0], i)) ;
Obj_Diagnostics[:x0_nokey] = penalty_nokey * sum(JuMP.value(x0_est[i]) for i in set[:i] if !haskey(cal[:x0], i)) ;
Obj_Diagnostics[:m0_nokey] = penalty_nokey * sum(JuMP.value(m0_est[i]) for i in set[:i] if !haskey(cal[:m0], i)) ;
Obj_Diagnostics[:md0_nokey] = penalty_nokey * sum(JuMP.value(md0_est[m,i])  for m in set[:m] for i in set[:i] if !haskey(cal[:md0], (m, i)))  ;
...
for i in set[:i]
    push!(constraint_diagnostic[:mkt_py], i => sum(JuMP.value(ys0_est[j,i]) for j in set[:j]) + JuMP.value(fs0_est[i])  - (sum(JuMP.value(ms0_est[i,m]) for m in set[:m]) + JuMP.value(y0_est[i])))
    push!(constraint_diagnostic[:mkt_pa], i => JuMP.value(a0_est[i])  - (sum(JuMP.value(id0_est[i,j]) for j in set[:j]) + sum(JuMP.value(fd0_est[i,fd]) for fd in set[:fd])))
    push!(constraint_diagnostic[:prf_a], i => JuMP.value(a0_est[i]) * (1 - cal[:ta0][i]) + JuMP.value(x0_est[i]) - ( JuMP.value(y0_est[i]) + JuMP.value(m0_est[i]) * (1 + cal[:tm0][i]) + sum(JuMP.value(md0_est[m,i]) for m in set[:m]) ))
end

for m in set[:m]
    push!(constraint_diagnostic[:mkt_pm], m => sum(JuMP.value(ms0_est[i,m]) for i in set[:i])  - (sum(JuMP.value(md0_est[m,i]) for i in set[:i])))
end

for j in set[:j]
    push!(constraint_diagnostic[:prf_y], j => sum(JuMP.value(ys0_est[j,i]) for i in set[:i])  - (sum(JuMP.value(id0_est[i,j]) for i in set[:i]) + sum(JuMP.value(va0_est[va,j]) for va in set[:va])))
end