...
function benchmark_disagg(k::Symbol, d_summ::Dict, d_bench::Dict, d_calc::Dict;
    ...
    ii = df_comp[:,:equal_keys] .* df_comp[:,:equal_values];
    ...
    d_summ[k] = any(.!ii) ? df_comp[.!ii,:] : true
    return d_summ
end
...
# ******************************************************************************************
disagg[:md0_sum] = combine_over(disagg[:md0], :m)
keys_calc = [:a0,:m0,:md0_sum,:ta0,:tm0,:s0_temp,:rx0_temp,:x0_temp]
vals_calc = [disagg[k] for k in keys_calc];

df = SLiDE._join_to_operate(dropmissing.(copy.(vals_calc)))
df = sort(edit_with(df, Rename.(find_oftype(df, AbstractFloat), keys_calc)))

df_empty = fill_zero((yr = set[:yr], r = set[:r], g = set[:g]));

disagg[:ta0_a0_rx0] = copy(df_empty)
disagg[:tm0_m0_md0] = copy(df_empty)
disagg[:s0_x0_rx0] = copy(df_empty)

disagg[:ta0_a0_rx0][!,:value] .= ((1 .- df[:,:ta0]) .* df[:,:a0]) + df[:,:rx0_temp]
disagg[:tm0_m0_md0][!,:value] .= ((1 .+ df[:,:tm0]) .* df[:,:m0]) + df[:,:md0_sum]
disagg[:s0_x0_rx0][!,:value]  .= df[:,:s0_temp] .- df[:,:x0_temp] + df[:,:rx0_temp]

disagg[:ta0_tm0] = disagg[:ta0_a0_rx0] - disagg[:tm0_m0_md0]

disagg[:ta0_tm0][!,:value] = round.(disagg[:ta0_tm0][:,:value]; digits = 10)
disagg[:s0_x0_rx0][!,:value]  = round.(disagg[:s0_x0_rx0][:,:value]; digits = 10)

# disagg[:ta0_a0_rx0] = dropmissing((1 - disagg[:ta0]) * disagg[:a0] + disagg[:rx0_temp])
# disagg[:tm0_m0_md0] = dropmissing((1 + disagg[:tm0]) * disagg[:m0]
#     + combine(groupby(disagg[:md0], [:yr,:r,:g]), :value => sum => :value))
# disagg[:ta0_tm0] = sort(dropmissing(disagg[:ta0_a0_rx0] - disagg[:tm0_m0_md0]))
# disagg[:s0_x0_rx0] = sort(dropmissing((disagg[:s0_temp] - (disagg[:x0_temp] - disagg[:rx0_temp]))))

# disagg[:ta0_a0_rx0][!,:value] = round.(temp[:ta0_a0_rx0][:,:value]; digits = 10)
# disagg[:tm0_m0_md0][!,:value] = round.(temp[:tm0_m0_md0][:,:value]; digits = 10)
# ******************************************************************************************

# "`diff`: Negative numbers still exist due to sharing parameter"
disagg[:diff] = copy(disagg[:ta0_tm0])
...
# ******************************************************************************************
keys_calc = [:a0,:m0,:md0_sum,:ta0,:tm0,:s0,:rx0,:x0]
vals_calc = [disagg[k] for k in keys_calc];
...
disagg[:ta0_a0_rx0][!,:value] .= ((1 .- df[:,:ta0]) .* df[:,:a0]) + df[:,:rx0]
disagg[:tm0_m0_md0][!,:value] .= ((1 .+ df[:,:tm0]) .* df[:,:m0]) + df[:,:md0_sum]
disagg[:s0_x0_rx0][!,:value]  .= df[:,:s0] .- (df[:,:x0] - df[:,:rx0])
...
disagg[:dd0max][!,:value] = min.(disagg[:ta0_tm0][:,:value], disagg[:s0_x0_rx0][:,:value])
...
disagg[:dd0min] = disagg[:ta0_tm0] - disagg[:dd0max]
...
#!    m0_(yr,r,g)*(1+tm0_(yr,r,g)) - sum(m,md0_(yr,r,m,g));
disagg[:nd0min] = disagg[:ta0_tm0] - disagg[:nd0max]
...
disagg[:nd0] = disagg[:ta0_tm0] - disagg[:dd0]
...
dm2 = dropmissing((disagg[:shrtrd] * disagg[:s0_x0_rx0]) - disagg[:dd0])
